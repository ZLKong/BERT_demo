...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.masked_retrain:
        ...
        for i in range(len(layers)):
            ...
            conv_mem = torch.chunk(conv, 16 , dim=0) #竖着分成16块
            for j in range(len(conv_mem)):
                conv_curr=conv_mem[j]
                print('convfrag[0].shape',conv_curr.shape)  #每一块的维度    
                print('convfrag[0].sum(0)',conv_curr.sum(0))   #各列之和
                for i in range(2):#range(conv_curr.shape[1]): #column number 
                    print('conv_curr.sum(0)[i]',conv_curr.sum(0)[i])  #第i列的和

    ...
    if args.rew:
        ...
        for i in range(len(layers)):
            ...
            if (args.sparsity_type == "block_filter"): # -libn
                ...
                if args.block_row_division != 0:
                    print("in loop",conv.shape[0], conv.shape[1])
                   # print('conv.shape[1]%block_row_width',conv.shape[1]%block_row_width)
                    if conv.shape[1]%args.block_row_division != 0 :
                        print('conv.shape[1]%block_row_division',conv.shape[1]%args.block_row_division)
                        print("the layer size is not divisible:",conv.shape[0], conv.shape[1])
                        # raise SyntaxError("block_size error")
                    block_row_division = int(conv.shape[1]/args.block_row_division)
               ...
               # print('mat sum',mat.sum(0))
               # print('mat.sum(0)',mat.sum(0))
               # print('len(mat)',len(mat))
               # print('mat[2930111]',mat[2930111])
               # print('mat[2930111][1]',mat[2930111][1])
               # mat[2930111][1] = torch.max(mat[2930111][1].cpu(),torch.tensor([0.]))
               # print('change mat[2930111][1]',mat[2930111][1])
               ...
               #            mat[j][i] = torch.max(mat[j][i].cpu(),torch.tensor([0.]))

                   # mat(0)=torch.max(mat(0),torch.tensor([0.]))
                #       print('new mat.sum(0)',mat.sum(0))
                #    else:
                #       print('no')
                #print(stoppp)

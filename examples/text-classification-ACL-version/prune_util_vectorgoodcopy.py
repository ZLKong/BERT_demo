...
def weight_pruning(args, weight, prune_threshold):
    ...
    if args.sparsity_type == "irregular":
       ...
       # print('!!! prune_threshold = ', prune_threshold)
        ...
        for i in range(shape[1]): # 多少列 column
        ...
        #if block_number==170:
            #print(stoppp)
            convfrag = torch.chunk(torch.tensor(conv[:,i]), block_number, dim=0)
            #print('len(convfrag)',len(convfrag))
            ...
            for vec in convfrag:
                ...
                vec2 = torch.chunk(torch.tensor(vec), 12, dim=0)   #竖着切
               ...
               #     else:
                ...
                whole_above_threshold[vec_num * 12:(vec_num+1)*12]= torch.from_numpy(above_threshold[:]).to(whole_above_threshold)

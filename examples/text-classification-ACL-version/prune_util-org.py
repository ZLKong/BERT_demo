    ...
    if args.sparsity_type == "irregular":
       ...
       # print('!!! prune_threshold = ', prune_threshold)
        ...
        if conv.shape[0]%block_row_width != 0 :
            print("the layer size is not divisible by block_row_width:",conv.shape[0], block_row_width)
            # raise SyntaxError("block_size error")
        block_number = int(conv.shape[0]/block_row_width)
        if block_number==0:  #for [2,768]
        ...
        # to construct the under_threshold matrix (under_threshold.shape = weight.shape)
        under_threshold_matrix = torch.tensor(np.tile(under_threshold.reshape((len(under_threshold),1)),block_number))

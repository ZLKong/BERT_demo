...
def weight_pruning(args, weight, prune_threshold):
    ...
    if args.sparsity_type == "irregular":
       ...
       # print('!!! prune_threshold = ', prune_threshold)
        ...
        print('block_norms',block_norms[0].shape)
        # Concatenate the row l2 norms:
        chunk_save=block_norms
        ...
        #print(torch.where(above_threshold))
        #print('above_threshold',above_threshold)
        #print('len(above_threshold)',len(above_threshold))
        #print('len(above_threshold:5)',len(above_threshold[:256]))
        #print(1*above_threshold[:5])
        #cc=1*above_threshold[:256]
        #if 1 in cc:
        #    print('yes')
        #else:
        #    print('no')
        #print('torch.nonzero(cc)',torch.nonzero(cc))
        #print('torch.count_nonzero(cc)',len(torch.nonzero(cc))==0)
        #myTensor=torch.where(above_threshold[:5])
        #print('myTensor.nelement() == 0',myTensor.nelement() == 0)
        #print('countTrue = tf.shape(torch.where(above_threshold[:5]))[0]',myTensor.shape[0])
        #print('len(print(torch.where(above_threshold[:5])))',len(torch.where(above_threshold[:5])))
        #cc=above_threshold[:5]==True
        #print('cc',block_norms[:5][cc].nelement() == 0)
        #print('above_threshold[:5]==True',above_threshold[:5]==True)


        # Step 3: to reproduce weight matrix using above_threshold:
        #print('above_threshold[101]',above_threshold[101])
        #print('above_threshold[0]',above_threshold[0])
        '''
        ...
        print('len(block_norms2)',len(block_norms2))
        for kk in range(len(chunk_save)):
            #print('kk',kk)
            ...
            #x_true=above_threshold[kk*block_row_width:(kk+1)*block_row_width]==True
            cc=1*above_threshold[kk*block_row_width:(kk+1)*block_row_width]
            if 1 in cc:
                #print('above_threshold[101]',above_threshold[101])
                above_threshold_chunked_matrix[kk*block_row_width:(kk+1)*block_row_width,:] = above_threshold[101] * np.ones((block_row_width,block_col_width)).astype('int')
            else:
                #print('no')   
                above_threshold_chunked_matrix[kk*block_row_width:(kk+1)*block_row_width,:] = above_threshold[0] * np.ones((block_row_width,block_col_width)).astype('int')
            #print(stoppp)
        '''
        ...
        print('above_threshold_matrix',len(above_threshold_matrix.shape))

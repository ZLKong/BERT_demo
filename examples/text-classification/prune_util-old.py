...
def weight_pruning(args, weight, prune_threshold):
    ...
    if args.sparsity_type == "irregular":
        ...
        shape = weight.shape
        conv = weight.reshape(shape[0], -1)
        print('weight.shape', conv.shape) #(512, 768)

        block_row_division = args.block_row_division
        block_row_width = args.block_row_width

        if block_row_division != 0:
            if conv.shape[0]%block_row_division != 0 :
                print("the layer size is not divisible by block_row_width:",conv.shape[0], block_row_division)
                # raise SyntaxError("block_size error")
            block_row_width = int(conv.shape[0]/block_row_division)
        else:
            if conv.shape[0]%block_row_division != 0 :
                print("the layer size is not divisible by block_row_division",conv.shape[0], block_row_width)
                # raise SyntaxError("block_size error")
        print('block_row_width int',block_row_width) #170
        if block_row_width==170:
             #block_row_width_add = 171
             convfrag = torch.chunk(torch.tensor(conv), 171, dim=0)
        else:
             convfrag = torch.chunk(torch.tensor(conv), block_row_width, dim=0)
        print('len(convfrag)',len(convfrag))  #128
        ...
        # to construct the under_threshold matrix (under_threshold.shape = weight.shape)
        print('before under block_row_division',block_row_division)
        print('int(conv.shape[0]/block_row_width)',int(conv.shape[0]/block_row_width))
        print('under_threshold.reshape((1,len(under_threshold)))',under_threshold.reshape((1,len(under_threshold))).shape)
        under_threshold_matrix = torch.tensor(np.tile(under_threshold.reshape((len(under_threshold),0)), int(conv.shape[0]/block_row_width)))
